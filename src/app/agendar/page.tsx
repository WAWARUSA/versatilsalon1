...
export default function AgendarPage() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // 5. Verificar que no haya conflictos con appointments existentes
      // Buscar appointments del mismo día y trabajador que puedan solaparse
      const startOfDay = new Date(startTime);
      startOfDay.setHours(0, 0, 0, 0);
      const endOfDay = new Date(startTime);
      endOfDay.setHours(23, 59, 59, 999);
      
      // Buscar appointments del mismo día
      ...
      const conflictingAppointments = existingAppointmentsSnapshot.docs.filter(doc => {
        const appData = doc.data();
        const appStartTime = appData.startTime?.toDate();
        const appEndTime = appData.endTime?.toDate();
        
        // Ignorar appointments cancelados
        if (appData.status === 'cancelled') return false;
        
        // Verificar si el appointment es con el mismo trabajador
        if (appData.performedBy !== performedBy) return false;
        
        // Verificar solapamiento de horarios
        if (!appStartTime || !appEndTime) return false;
        
        // Verificar si hay solapamiento:
        // - El nuevo appointment inicia dentro del existente
        // - El nuevo appointment termina dentro del existente
      ...
      if (conflictingAppointments.length > 0) {
        const conflictingApp = conflictingAppointments[0].data();
        const conflictingStart = conflictingApp.startTime?.toDate();
        const conflictingTime = conflictingStart ? 
          `${String(conflictingStart.getHours()).padStart(2, '0')}:${String(conflictingStart.getMinutes()).padStart(2, '0')}` : 
          'horario desconocido';
        throw new Error(`El horario seleccionado (${selectedTime}) ya está ocupado para ${performedBy} desde las ${conflictingTime}. Por favor, selecciona otro horario.`);
      }

      // 6. Crear el appointment
