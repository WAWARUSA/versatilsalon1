...
export default function StepReview({
  ...
  useEffect(() => {
    const fetchServiceName = async () => {
      if (!selectedService || !db) {
        // Si no hay servicio seleccionado, usar el mapeo estático
        setServiceName(serviceNames[selectedService] || selectedService || 'No seleccionado');
        return;
      }
      ...
      try {
        ...
        if (serviceSnap.exists()) {
          ...
          setServiceName(serviceData.name || serviceNames[selectedService] || selectedService);
        } else {
          // Si no se encuentra por ID, usar el mapeo estático
          setServiceName(serviceNames[selectedService] || selectedService);
        }
      } catch (error) {
        console.error('Error cargando nombre del servicio:', error);
  ...
  const formatDate = (dateStr: string) => {
    ...
    // Crear la fecha usando los componentes directamente (mes - 1 porque los meses van de 0-11)
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#151414] border border-[#c9a857]/20 rounded-xl p-6">
          ...
          <p className="text-gray-300">{serviceName}</p>
        </div>
